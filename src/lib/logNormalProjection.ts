...
function impliedProbabilityFromAmericanOdds(odds: number): number {
    // Assumes standard American odds.
...
export function projectMeanLogNormalFromOverUnder(params: {
    line: number
    ...
    const pOver = clamp01(fair.p1)

    // For X ~ LogNormal(mu, sigma), we have:
    // P(X > line) = pOver
    // => P(X <= line) = 1 - pOver
    // => (ln(line) - mu) / sigma = Φ^{-1}(1 - pOver)
    const z = normalInv(1 - pOver)
    const mu = Math.log(line) - sigma * z
    const mean = Math.exp(mu + 0.5 * sigma * sigma)

    // Keep display friendly for tables
